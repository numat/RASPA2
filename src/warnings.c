/*****************************************************************************************************
    ...
    This file 'warnings.c' is part of RASPA.
...
/*
    ...
    UndefinedVDWPotential=FALSE;
    for(i=0;i<NumberOfAdsorbateMolecules[k];i++)
    {
      for(j=0;j<Adsorbates[k][i].NumberOfAtoms;j++)
      {
        TypeA=Adsorbates[k][i].Atoms[j].Type;

        for(f1=0;f1<Framework[k].NumberOfFrameworks;f1++)
        {
          for(l=0;l<Framework[k].NumberOfAtoms[f1];l++)
          {
            TypeB=Framework[k].Atoms[f1][l].Type;
            if(PotentialType[TypeA][TypeB]==ZERO_POTENTIAL)
              UndefinedVDWPotential=TRUE;
          }
        }
      }
    }
    if(UndefinedVDWPotential)
      Warnings[k][NumberOfWarnings[k]++]=OMITTED_HOST_ADSORBATE_VDW_INTERACTIONS;
    ...
    for(i=0;i<NumberOfCationMolecules[k];i++)
    {
      for(j=0;j<Cations[k][i].NumberOfAtoms;j++)
      {
        TypeA=Cations[k][i].Atoms[j].Type;
    ...
    if(UndefinedVDWPotential)
      Warnings[k][NumberOfWarnings[k]++]=OMITTED_HOST_CATION_VDW_INTERACTIONS;
    ...
    for(i=0;i<NumberOfAdsorbateMolecules[k];i++)
    {
      ...
      {
        TypeA=Adsorbates[k][i].Atoms[j].Type;
        for(l=0;l<NumberOfAdsorbateMolecules[k];l++)
        {
          for(m=0;m<Adsorbates[k][l].NumberOfAtoms;m++)
          {
            TypeB=Adsorbates[k][l].Atoms[m].Type;
            if(PotentialType[TypeA][TypeB]==ZERO_POTENTIAL)
              UndefinedVDWPotential=TRUE;
          }
        }
    ...
    if(UndefinedVDWPotential)
      Warnings[k][NumberOfWarnings[k]++]=OMITTED_ADSORBATE_ADSORBATE_VDW_INTERACTIONS;
*/
